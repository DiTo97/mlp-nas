...
class MLPGenerator(MLPSearchSpace):
    ...
    def compile_model(self, model):
        ...
        if self.mlp_optimizer == 'SGD':
            optim = optimizers.SGD(lr=self.mlp_lr, decay=self.mlp_decay,
                                   momentum=self.mlp_momentum)
        else:
            optim = getattr(optimizers, self.mlp_optimizer)(lr=self.mlp_lr,
    ...
    def update_weights(self, model):
        ...
        for layer in model.layers:
            if 'flatten' in layer.name:
                layer_configs.append(('flatten')) # Standardize the way flatten layers are stored,
                                                  # as they do not change the dimensions of weights.
            # Skip dropout layers to fasten computation
            elif 'dropout' not in layer.name:
                layer_configs.append((layer.get_config()['units'],
                                      layer.get_config()['activation']))
        
        config_ids = [] # Extract the model's valuable bigrams
        for i in range(1, len(layer_configs)):
            config_ids.append((layer_configs[i - 1],
                               layer_configs[i]))
        ...
        for i, layer in enumerate(model.layers):
            if 'dropout' not in layer.name:
                ...
                bigram_ids = self.shared_weights['bigram_id'].to_numpy() # Get pre-saved weights
                
                # Update bigram weights if already present...
                if list(config_ids)[j] in bigram_ids.tolist():
                    k = bigram_ids.tolist().index(list(config_ids)[j]) # Find index k of bigram
                    self.shared_weights.at[k, 'weights'] = layer.get_weights()
                else:
                    # ....else create a new bigram and add it to the dictionary
                    self.shared_weights = self.shared_weights.append({'bigram_id': config_ids[j],
                                                                      'weights': layer.get_weights()},
                                                                      ignore_index=True)
                # Updated only when non-dropout layer, such that the
                # two lists stay synchronized while looping                    
                j += 1
    ...
    def set_model_weights(self, model):
        ...
        for layer in model.layers:
            if 'dropout' not in layer.name:
                ...
                if list(config_ids)[j] in bigram_ids.tolist(): # If the bigram already exists, load the weights.
                    k = bigram_ids.tolist().index(list(config_ids)[j])
                    print("Transferring weights for layer:", config_ids[j])
                    layer.set_weights(self.shared_weights['weights'].values[k])

                # Update only on non-dropout layers
                j += 1
